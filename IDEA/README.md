...
本插件 = 码字计数 + 时间计数 + 固定时间休息提醒 + 强打字动效 + 幽默文案，**功能与 PyCharm 版完全一致**。
编译目标：`IntelliJ IDEA Community 2021.3+`（`build.gradle.kts` 里 `type.set("IC")`）。

## 构建 & 安装（本地从磁盘装）
1. 用 IntelliJ IDEA 打开本文件夹，作为 Gradle 项目导入（会自动下载 IDEA SDK）。
